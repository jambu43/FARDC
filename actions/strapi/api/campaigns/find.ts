...
import {getCollection,getCollectionById} from "@/actions/strapi/services/server";
...
export const getCampaignsByUser = async () => {
  ...
  const campaigns = await getCollection(`campaigns?filters[users_permissions_user][documentId][$eq]=${userId}&sort=createdAt:desc`);
...
export const getStatsCampaigns = async () => {
  ...
  const today = new Date();
  const userId = user?.id;
  const allcampaigns = await getCollection(`campaigns?filters[users_permissions_user][documentId][$eq]=${userId}&sort=createdAt:descpagination[pageSize]=200`);
 const campaignsActive = allcampaigns?.data?.filter((item: any) => {
   const endDate = new Date(item.endDate);
   return endDate > today;
 }).length
 const totalAmountCollected = allcampaigns?.data?.reduce((sum: number, campaign: any) => sum + campaign?.amountCollected, 0);
 const wincampaigns = allcampaigns?.data?.filter((item: any) => {
  ...
  return {
    ...
    reussite:Math.round((wincampaigns/allcampaigns?.meta?.pagination?.total)*100),
  };
